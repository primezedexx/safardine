...
import { 
  ...
  Sparkles, Flame, ChefHat, Tag, Eye, Heart, Upload, Loader2, X, ChevronDown, ChevronUp, Trash2, Save
} from 'lucide-react'
import { uploadRestaurantImage } from '@/app/(auth)/actions'

interface EditMenuFormClientProps {
  item: any
...
export default function EditMenuFormClient({ item, updateMenuItem, deleteMenuItem, currency, onClose }: EditMenuFormClientProps) {
  ...
  // Status and Availability mapped from database 'available'
  const [status, setStatus] = useState(item.available !== false ? 'Active' : 'Inactive')
  const [availability, setAvailability] = useState(item.available !== false ? 'Available' : 'Unavailable')
  
  // Food type parsed from item tags
  const initialFoodType = item.tags?.includes('Vegan') 
    ? 'Vegan' 
  ...
  const [ingredients, setIngredients] = useState(item.ingredients ? item.ingredients.join(', ') : '')
  
  // Clean initial tags by filtering out Food Type tags
  const initialTagsStr = item.tags
    ? item.tags.filter((t: string) => t !== 'Veg' && t !== 'Non-Veg' && t !== 'Vegan' && t !== 'Vegetarian').join(', ')
    : ''
  const [tags, setTags] = useState(initialTagsStr)
  const [calories, setCalories] = useState(item.calories ? item.calories.toString() : '')

  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isDeleting, setIsDeleting] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [isUploading, setIsUploading] = useState(false)
  const [uploadedImageUrl, setUploadedImageUrl] = useState<string | null>(item.image_url || null)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let tagsList = tags ? String(tags).split(',').map((t: string) => t.trim()).filter(Boolean) : []
      tagsList = tagsList.filter((t: string) => t !== 'Veg' && t !== 'Non-Veg' && t !== 'Vegan' && t !== 'Eggetarian')
  ...
  const handleDelete = async () => {
    if (!confirm('Are you absolutely sure you want to delete this menu item? This action is irreversible.')) {
      return
    }
  ...
  const tagsList: string[] = tags ? String(tags).split(',').map((t: string) => t.trim()).filter(Boolean) : []
  const ingredientsList: string[] = ingredients ? String(ingredients).split(',').map((i: string) => i.trim()).filter(Boolean) : []
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 bg-[#FAFAFA] border border-slate-100 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.05)] overflow-hidden relative">
            ...
            <div className="px-6 py-5 border-b border-[#E5E7EB]/40 flex items-center justify-between bg-slate-50">
              <div>
                <h1 className="text-xl font-bold text-slate-900">Edit Menu Item</h1>
                <p className="text-[11px] font-semibold text-slate-400 mt-0.5 uppercase tracking-wider">Modify or remove your culinary creation</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-1.5 border border-red-200 text-red-500 hover:bg-red-50 disabled:opacity-50 text-[10px] font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer flex items-center gap-1.5"
                  title="Delete item"
                >
